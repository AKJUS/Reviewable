...
- <kbd>Fix:</kbd> on EMU instances, filter out non-EMU usernames from mentions and completion condition results, and fail on attempts to add a non-EMU assignee or reviewer. <!-- client 7872 -->
- <kbd>New:</kbd> let agents interact with Reviewable via a CLI or an MCP server. <!-- client 7870 -->
  ...
  > with a minimal API surface that just covers responding to code reviews
...
- <kbd>Fix:</kbd> don't crash when creating a code block in a language Reviewable can't highlight. <!-- client 7859 -->
...
- <kbd>Fix<i>(saas)</i>:</kbd> don't insert a newline into the summary when a comment starts with an icon or emoji. <!-- client 7834 -->
...
- <kbd>New:</kbd> We also added a new [Discussion matrix](https://docs.reviewable.io/reviews#discussions), because hunting for comments was getting old. <!-- client 7827 -->
  > The new Discussion matrix organizes and provides key details for every discussion, making it easy to triage busy reviews.
- <kbd>Fix<i>(enterprise)</i>:</kbd> fix a race condition that could lead to `Encryption not set up` errors when opening multiple Reviewable tabs when the instance is running in private mode on a shared worker. <!-- client 7827 -->
...
- <kbd>Adm<i>(saas)</i>:</kbd> Slack account links from GitHub profiles are now synced more frequently for members of organizations that have a Slack workspace connected. <!-- server 4887 -->
