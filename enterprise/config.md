...
We recommend running in a VM with 2GB of memory available to the server — much less than that and you might run out of memory when handling some big pull requests.  If you can allocate 6GB+ to the VM then you'll also want to set `NODE_OPTIONS="--max-old-space-size=4096"` to make Node take advantage of the extra memory, otherwise it'll just go unused.  The server is single-threaded, so one vCPU (or even a fraction of one) is sufficient, and the processes are more network than CPU intensive anyway.  On Google Cloud Platform, the `g1-small` instance type is a good fit, on AWS a `t2.small`, and on Azure an `A1`.
...
* `REVIEWABLE_AGENT_HMAC_KEY`: An arbitrary secret string that will be used when hashing agent API keys.  You can use `openssl rand -hex 32` to generate one.  If no value is provided then all agent API features will be disabled.  Rotating this key will immediately invalidate all API keys (forcing users to request new ones), preventing new sessions from starting and breaking existing session after no longer than one hour.
...
* `REVIEWABLE_CRON_CHILDREN_KEYS_WARNING_THRESHOLD`: A [human-readable](https://github.com/vercel/ms) duration (for example, `10s`) that controls when Reviewable treats a cron job's top-level Firebase `childrenKeys()` scan as slow.  Every scan is still logged and reported via `statsd` if configured, but scans at or above this threshold will additionally be flagged in the structured logs as slow and reported to Sentry as warning-level events.  Defaults to `30s`.
* `REVIEWABLE_REPORTING_ENDPOINT`: The server URL to which browsers should [send reports](https://developer.chrome.com/docs/capabilities/web-apis/reporting-api), such as page crash or network failure reports.
...
* `GAE_SERVICE` or `GAE_VM`: If non-empty (any value) servers will handle requests to `/_ah/health` and `/_ah/ready`; the former will respond with `200` whenever possible, the latter will respond with `200` unless the server is shutting down, in which case it'll respond with `503` instead.  If you set up a readiness watcher make sure it gives the server 1 minute to perform a controlled shutdown (or complete an exclusive task and become ready again) before you force it to restart.  Servers will also trust headers inserted by the last HTTP proxy.  (If you were previously using `/_ah/stop` for graceful shutdown requests, please now send a `SIGTERM` instead.)
...
![Video tutorial](images/slack.gif)
